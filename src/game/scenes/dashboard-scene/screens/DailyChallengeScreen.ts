...
import { loadLevel, startJourneyDailyChallengeMode } from '@/modules/match/actions/match'
...
import PlayButton from './mini-journey/PlayButton'
import { TARGET_SCORE } from '../../journey-scene/constant/target'
...
export default class DailyChallengeScreen extends GameCore.Screen {
    ...
    private panelCollection: Phaser.GameObjects.Image
    private tileCollection: Phaser.GameObjects.Image
    ...
    private loadLevelConfig(): void {
        try {
            const { key, mapJson } = getFileConfigDailyChallengeMode()
    ...
    private createTitle(): void {
        ...
        const text = this.scene.make.image({
            key: KEY,
            frame: FRAME.TEXT_DAILY_CHALLENGE,
        })
        ...
        Phaser.Display.Align.In.Center(this.buttonClose, this.title, 138.5, -26)
    ...
    private createTextScore(): void {
        this.textScore = this.scene.make.bitmapText({
            font: FONTS.NUMBER_DARK.KEY,
            size: this.scene.fontSize(54),
            origin: { x: 0.5, y: 0.5 },
            text: `0`,
        })
    ...
    private createTargetItems(): void {
        ...
        this.tileCollection = this.scene.make.image({
            key: KEY,
            frame: FRAME.TEXT_COLLECT,
        })

        const { width, height } = this.tileCollection

        this.tileCollection.setWorldSize(width / imageScale, height / imageScale)

        this.panelCollection = this.scene.make.image({
            key: KEY,
            frame: FRAME.WOOD_PANEL,
        })

        this.panelCollection.setWorldSize(200, 37)

        this.popup.add([this.tileCollection, this.panelCollection])

        Phaser.Display.Align.In.Center(this.tileCollection, this.popup, 0, -155)
        Phaser.Display.Align.In.Center(this.panelCollection, this.popup, 0, -125)
    }

    private drawTargetItems(): void {
        const { key } = getFileConfigDailyChallengeMode()
    ...
    private updateTargetType(): void {
        ...
        const { value: score } = properties[0]
        const { value: targetType } = properties[1]
        const imageScale = this.scene.world.getPixelRatio()

        if (targetType === TARGET_SCORE) {
            this.panelCollection.setVisible(false)
            this.tileCollection.setFrame(FRAME.TEXT_TARGET_SCORE)
            const { width, height } = this.tileCollection

            this.tileCollection.setWorldSize(width / imageScale, height / imageScale)
            Phaser.Display.Align.In.Center(this.tileCollection, this.popup, 0, -150)
            this.textScore.setText(`${score}`).setVisible(true)
            Phaser.Display.Align.In.Center(this.textScore, this.popup, 0, -125)
            return
        }

        this.panelCollection.setVisible(true)
        this.tileCollection.setFrame(FRAME.TEXT_COLLECT)
        const { width, height } = this.tileCollection
    ...
    public handlePlay() {
        ...
        this.popupFadeOutAnimation.next(() => {
            ...
            // this.scene.storage.dispatch(clearJourneyMatchData())

            this.scene.storage.dispatch(startJourneyDailyChallengeMode())

            this.scene.storage.dispatch(startJourneyModeGame())

            this.scene.analytics.event(AnalyticsEvents.DAILY_CHALLENGE_BEGIN)
        })
    }
