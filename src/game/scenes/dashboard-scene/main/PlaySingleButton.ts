...
class PlaySingleButton extends Button {
    private icon: Phaser.GameObjects.Image
    private text: Phaser.GameObjects.Image

    constructor(scene: Phaser.Scene) {
        super(scene, KEY, FRAME.BUTTON_GREEN_LARGE)

        this.button.setWorldSize(242, 70)

        this.setName('PlaySingle')
    ...
    private createIcon() {
        this.icon = this.scene.make.image({
            key: KEY,
            frame: FRAME.ICON_PLAY_GREEN,
        })

        this.icon.setWorldSize(31, 31)
...
export default PlaySingleButton
