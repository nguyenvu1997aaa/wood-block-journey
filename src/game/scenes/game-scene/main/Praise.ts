...
export default class Praise extends Phaser.GameObjects.Container {
    ...
    private createTweenScoreText() {
        const state = this.scene.storage.getState()
        const gamePlayMode = getGameplayMode(state)

        let { x, y } =
            this.scene.layoutManager.objects.header.mainScoreBlock.scoreText.getWorldPosition()
        ...
        if (
            ...
            const { x: myScoreX, y: myScoreY } =
                this.scene.layoutManager.objects.header.myScoreBlock.textScore.getWorldPosition()
            x = myScoreX
            y = myScoreY
        }
    ...
    playAnimShowScoreText() {
        ...
        if (this.tweenHideScoreText && this.tweenHideScoreText.isPlaying()) {
            ...
            if (mode === MATCH_MODE_SINGLE) {
                ...
                currentScore = header.myScoreBlock.score

                header.myScoreBlock.setScore(currentScore + (this.score || 0))
                header.myScoreBlock.setScoreText(currentScore + (this.score || 0))
            }
        }
    ...
    handlePraiseFrameText(x: number, y: number, level: number) {
        ...
        this.praiseFrame.setPosition(positionX, y - 40)
    ...
    getMainFrameByLevel(level: number) {
        if (level > 6) {
            return SPRITES.EFFECTS.FRAME.TEXT_EXCELLENT
        }
