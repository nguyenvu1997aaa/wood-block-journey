        ...
        if (isCaptureLine) {
            ...
            for (let x = 0; x < cols; x++) {
                if (!captureCells.some((item) => item.x === x && item.y === y)) {
                    captureCells.push({ x, y, frame: board[y][x].frame })
                }
            }
        }
    }
...
export const getFenCharacterByCode = (code: string) => {
    ...
    switch (code) {
        ...
        // CASE DIAMOND
        case DIAMOND_10: {
            character = FEN_DIAMOND_10
            break
        }
        case DIAMOND_11: {
            character = FEN_DIAMOND_11
            break
        }
        case DIAMOND_12: {
            character = FEN_DIAMOND_12
            break
        }
        case DIAMOND_13: {
            character = FEN_DIAMOND_13
            break
        }
        case DIAMOND_14: {
            character = FEN_DIAMOND_14
            break
        }
        case DIAMOND_15: {
            character = FEN_DIAMOND_15
            break
        }
        // END CASE DIAMOND
        ...
        case T_PIECE_TYPE: {
            character = FEN_T_PIECE
            break
        }
        case CROSS_PIECE_2_TYPE: {
            character = FEN_CROSS_PIECE_2
            break
        }
        case CROSS_PIECE_3_TYPE: {
            character = FEN_CROSS_PIECE_3
            break
        }
        case CROSS_PIECE_4_TYPE: {
            character = FEN_CROSS_PIECE_4
            break
        }
        case CROSS_PIECE_5_TYPE: {
            character = FEN_CROSS_PIECE_5
            break
        }
        case U_PIECE_TYPE: {
            character = FEN_U_PIECE
            break
        }
...
export const getCodeByFenCharacter = (fenCharacter: string): string => {
    ...
    switch (fenCharacter) {
        ...
        case FEN_CROSS_PIECE_2: {
            code = CROSS_PIECE_2_TYPE
            break
        }
        case FEN_CROSS_PIECE_3: {
            code = CROSS_PIECE_3_TYPE
            break
        }
        case FEN_CROSS_PIECE_4: {
            code = CROSS_PIECE_4_TYPE
            break
        }
        case FEN_CROSS_PIECE_5: {
            code = CROSS_PIECE_5_TYPE
            break
        }
        case FEN_U_PIECE: {
            code = U_PIECE_TYPE
...
export const decodeRowByString = (str: string) => {
    ...
    for (let i = 0; i < tmp.length; i++) {
        const code = tmp[i]
        const frame = getCodeByFenCharacter(code)

        if (code === 'x') {
            data.push({ value: 0, frame: TRANSPARENT })
        } else {
            data.push({ value: 1, frame })
        }
    }
...
export const decodePiece = (str: string) => {
    ...
    return { type, direction, isReverse, fenPiece: tmp[1] }
}
