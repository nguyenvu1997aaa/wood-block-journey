...
export default class InputManager {
    constructor(manager) {
        ...
        this.isEnable = manager.isEnable
    }
    create(_scene, _debugBox, _folder, _camera, _typeSort) {
        this.scene = _scene
        ...
        this.createEnableCmd(_scene, _typeSort)
    }
    update() {
    ...
    createEnableCmd(_scene, _typeSort) {
        _scene.input.keyboard.on('keyup-X', () => {
            this.isEnable = !this.isEnable
            this.manager.isEnable = this.isEnable
            _typeSort.setEnable(this.isEnable)
        })
    }
    ...
    createOverEvent(_scene, _debugBox, _folder) {
        ...
        _scene.input.on('gameobjectover', (_pointer, _gameObj) => {
            if (!this.manager.isEnable) return
    ...
    createDetailEvent(_scene, _debugBox, _folder) {
        _scene.input.keyboard.on('keyup-D', () => {
            let tmpFocusGameObj = _debugBox.getFocusGameObj()
            if (
                // chck if focus valid & shift key down
                tmpFocusGameObj &&
    ...
    chckCommandKeyReleased(_pointer) {
        let tmpBool
        if (
            (this.getCursorKey().shift.isDown && _pointer.leftButtonReleased()) || // shift + mouse left click or
    ...
    chckCommandKeyDown(_pointer) {
        let tmpBool
        if (
            (this.getCursorKey().shift.isDown && _pointer.leftButtonDown()) || // shift + mouse left click or
