...
export default class TypeSortManager {
    ...
    createFocusFolderTryException(_objList, _obj) {
        if (_obj.type !== 'Graphics' && _obj.type !== 'Container' && _obj.type !== 'Group') {
            ...
            this.createFocusFolderTryExceptionGroup(_objList, _obj)
        }
    }
    ...
    createFocusFolderTryExceptionGroup(_objList, _obj) {
        let tmpList = _obj.getChildren()
        let tmpLength = tmpList.length
        for (var i = 0; i < tmpLength; i++) {
            this.createFocusFolderTryException(tmpList, tmpList[i])
            this.createFocusFolderSetObj(_objList, tmpList[i])
    ...
    createFocusFolderTryExceptionNotContainer(_obj) {
        try {
            if (this.isEnable && _obj.input && !_obj.input.enabled) {
                _obj.setInteractive()
                console.log('set')
                this.newObjectInteractive.push(_obj)
            }
        } catch (e) {
            //
        }
    ...
    setEnable(value = false) {
        this.newObjectInteractive.forEach((o) => {
            if (!value) {
    ...
    createFocusFolderSetObj(_objList, _obj) {
        if (_obj.guiIdx != undefined) return
        ...
        _obj.GUI_FOCUS_ONOFF = function () {
            ...
            if (!this.isFocusOnGUI) {
                tmpObjs.folder.cross2FocusObj(_obj)
            }
    ...
    chckParentContainer(_obj, _objList, _tmpGUIIdx) {
        ...
        if (tmpPC) {
            ...
            _obj.GUI_CONTAINER = () => {
                this.folder.closeThisNopenParentContainer
                    .bind(_obj, [_tmpGUIIdx, tmpPC, this.folder, this.debugBox, this])
                    .call()
                let tmpObjFolder = this.folder.getCustomFoldersInFolder()

                this.chckObjType(undefined, tmpPC.guiIdx, tmpObjFolder[tmpPC.guiIdx], tmpPC)
            }
        }
    }
    createCustomInDetail(_obj, _objList, _tmpGUIIdx) {
        this.folder.add2CustomFolder()
        // this.chckObjType(this.folder.getCustomFolder(), _tmpGUIIdx, tmpFolderInCustom, _obj)
    }

    chckObjType(_custom, _idx, _folderInCustom, _obj) {
        // check each of objs type
        if (this.createdList.indexOf(_idx) >= 0) return
        this.createdList.push(_idx)
        let tmpGameObj = _obj
    ...
    createBackFunc(_idx, _folderInCustom, _gameObj) {
        // create back 2 basic function
        _folderInCustom.addButton({ title: 'GUI_BACK' }).on('click', _gameObj.GUI_BACK)
    }
    createFocusToggle(_folderInCustom, _gameObj) {
        if (_gameObj.type !== 'Graphics') {
            _folderInCustom
                .addButton({ title: 'GUI_FOCUS_ONOFF' })
                .on('click', _gameObj.GUI_FOCUS_ONOFF)
        }
    }
    createConsoleFunc(_idx, _folderInCustom, _gameObj) {
        try {
            _folderInCustom.addButton({ title: 'GUI_CONSOLE' }).on('click', _gameObj.GUI_CONSOLE)
        } catch (e) {
            //
        }
    }
    createContainerFunc(_idx, _folderInCustom, _gameObj) {
        if (_gameObj.GUI_CONTAINER) {
            _folderInCustom
                .addButton({ title: 'GUI_CONTAINER' })
                .on('click', _gameObj.GUI_CONTAINER)
        }
    }
        ...
        if (tmpType === true) {
            ...
            switch (tmpBodyType) {
                case 'string': // matter
                    ...
                    _folderInCustom.addInput(tmpObj, 'type')
    ...
    createCommonBack(_folderInCustom, _gameObj) {
        const pos = create2DObject(_gameObj, 'x', 'y', 'position')
        this.tryCatch(_folderInCustom, pos, 'position', 'listen')
        const size = create2DObject(_gameObj, 'width', 'height', 'size')
        this.tryCatch(_folderInCustom, size, 'size', 'listen')
        const scale = create2DObject(_gameObj, 'scaleX', 'scaleY', 'scale')
        this.tryCatch(_folderInCustom, scale, 'scale', 'listen')
        if (_gameObj.displayWidth && _gameObj.displayHeight) {
            const displaySize = create2DObject(
                _gameObj,
                'displayWidth',
                'displayHeight',
                'displaySize'
            )
            this.tryCatch(_folderInCustom, displaySize, 'displaySize', 'listen')
        }
        const origin = create2DObject(_gameObj, 'originX', 'originY', 'origin')
        this.tryCatch(_folderInCustom, origin, 'origin', 'listen')
    ...
    createContainer(_idx, _folderInCustom, _gameObj) {
        ...
        let tmpList = _folderInCustom.addFolder({ title: 'list' })
        tmpList.addMonitor(_gameObj.list, 'length')
        for (var i = 0; i < _gameObj.list.length; i++) {
            let _obj = _gameObj.list[i]
            let gotoDetail = () => {
                this.folder.cross2FocusObj(_obj)
            }
            let childFolder = {
                GUIIdx: _obj.guiIdx,
                name: _obj.name,
                type: _obj.type,
                texture: _obj.texture && _obj.texture.key ? _obj.texture.key : '',
                frame: _obj.frame && _obj.frame.name ? _obj.frame.name : '',
                GUI_GO_2_DETAIL: gotoDetail,
            }

            let tmpFocus = tmpList.addFolder({ title: 'child ' + i }) // add to Parent Obj folder
            tmpFocus.addMonitor(childFolder, 'GUIIdx')
            tmpFocus.addMonitor(childFolder, 'name')
            tmpFocus.addMonitor(childFolder, 'type')
            tmpFocus.addMonitor(childFolder, 'texture')
            tmpFocus.addMonitor(childFolder, 'frame')
            tmpFocus
                .addButton({ title: 'GUI_GO_TO_DETAIL' })
                .on('click', childFolder.GUI_GO_2_DETAIL)
        }
    ...
    createListText(_idx, _folderInCustom, _gameObj) {
        console.log({ _idx, _folderInCustom, _gameObj })
        _folderInCustom.addInput(_gameObj, 'text')

        const style = _folderInCustom.addFolder({ title: 'style' })

        this.tryCatch(style, _gameObj.style, 'align')
        this.tryCatch(style, _gameObj.style, 'fontFamily')
        this.tryCatch(style, _gameObj.style, 'fontStyle')
        this.tryCatch(style, _gameObj.style, 'fontSize', 'listen')
        this.tryCatch(style, _gameObj.style, 'color')
        this.tryCatch(style, _gameObj.style, 'backgroundColor')
        this.tryCatch(style, _gameObj.style, 'stroke')
        this.tryCatch(style, _gameObj.style, 'strokeThickness')
        this.tryCatch(style, _gameObj.style, 'shadowStroke')
        this.tryCatch(style, _gameObj.style, 'shadowColor')
        this.tryCatch(style, _gameObj.style, 'shadowBlur')
        this.chckEndSorting(_idx)
    }
    ...
    createMatterBody(_folderInCustom, _gameObj) {
        let tmpBody = _folderInCustom.addFolder({ title: 'body' })
    ...
    createTextureNFrame(_idx, _folderInCustom, _gameObj, _srcObj) {
        ...
        tmpFrame.setFrame = () => {
            let tmpName = tmpF.value
            _gameObj.setFrame(tmpName)
        }
        tmpT = _folderInCustom.addBlade({
            view: 'list',
            label: 'textureList',
            options: tmpTexture.textureList.map((el) => {
                return { text: el, value: el }
            }),
            value: tmpGameObjTexture,
        })
        _folderInCustom.addButton({ title: 'setTexture' }).on('click', tmpTexture.setTexture)
        tmpF = _folderInCustom.addBlade({
            view: 'list',
            label: 'frameList',
            options: tmpFrame.frameList.map((el) => {
                return { text: el, value: el }
            }),
            value: tmpFrame.frameList[0],
        })
        _folderInCustom.addButton({ title: 'setFrame' }).on('click', tmpFrame.setFrame)
    }
    createAnims(_idx, _folderInCustom, _gameObj) {
        ...
        let tmpAnims = _folderInCustom.addFolder({ title: 'anims' })
        // play animation
        tmpAnimList = tmpAnims.addBlade({
            view: 'list',
            label: 'playList',
            options: tmpAnimPlay.playList.map((el) => {
                return { text: el, value: el }
            }),
            value: tmpAnimPlay.playList[0],
        })
        tmpAnims.addButton({ title: 'play' }).on('click', tmpAnimPlay.play)
    ...
    createAllThePropertyOfObj(_folderInCustom, _nameStr, _listenObj) {
        let tmpFolder = _folderInCustom.addFolder({ title: _nameStr })
    ...
    tryCatch(_guiObj, _obj, _property, _cmd, _customFunction) {
        ...
        try {
            _guiObj.addInput(_obj, _property)
        } catch (e) {
            //
        }
        // if (tmpAddFunc) {
...
var create2DObject = (obj, x, y, name) => {
    return {
        [name]: {
            get x() {
                return obj[x]
