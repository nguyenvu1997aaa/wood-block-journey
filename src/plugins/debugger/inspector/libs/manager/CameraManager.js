export default class CameraManager {
    ...
    set2defaultZoom() {
        ...
        this.mainCamera.zoomTo(this.defaultCamera.zoom, 0)
        this.mainCamera.scrollX = this.defaultCamera.scrollX
        this.mainCamera.scrollY = this.defaultCamera.scrollY
        this.dragConfig.scrollX = this.defaultCamera.scrollX
        this.dragConfig.scrollY = this.defaultCamera.scrollY
    }
    createFollowEvent(_scene, _debugBox) {
        ...
        _scene.input.keyboard.on('keyup-A', (_pointer, _gameObj) => {
            this.tmpFocusGameObj = _debugBox.getFocusGameObj()
            if (this.chckCmdShiftKeyDown() && this.tmpFocusGameObj) {
                if (!this.isFollowing) {
    ...
    updateDrag() {
        if (this.isFollowing) {
            let { x, y } = this.getWorldPosition(this.tmpFocusGameObj)
