...
export default class DebugBoxManager {
    ...
    createSetting(_scene) {
        ...
        this.graphics.setDepth(99999999)
    ...
    setDebugBox(_target, _gameObj) {
        ...
        if (_gameObj.getBounds) {
            this.graphics.strokeRectShape(_gameObj.getBounds())
        } else if (this.validateDrawAbleObject(_gameObj)) {
            const { x, y } = this.getWorldPosition(_gameObj)
            const { width, height } = _gameObj
            const { originX = 0.5, originY = 0.5 } = _gameObj
            const { scaleX = 1, scaleY = 1 } = _gameObj
    ...
    setFocusPerformance(_gameObj, _folder) {
            ...
            onUpdate: () => {
                if (_gameObj.isFocusOnGUI) {
                    if (tmpTwConfig.isTint) {
                        ...
                        _gameObj.setTint(
                            Phaser.Display.Color.GetColor(tmpValue, tmpValue, tmpValue)
                        )
                    } else {
                        let tmpValue = _gameObj.focusTw.getValue()
    ...
    clear() {
        this.clearFocus()
        this.clearFocusGameObj()
        this.clearOverGameObj()
    }
}
