...
var Frame = require('phaser/textures/Frame')
var TextureSource = require('phaser/textures/TextureSource')
...
var Texture = new Class({
    ...
    copy: function (texture) {
        this.customData = texture.customData
        this.dataSource = texture.dataSource
        this.firstFrame = texture.firstFrame
        this.manager = texture.manager
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            var targetFrame = this.frames[key]
            var sourceFrame = texture.frames[key]

            if (!targetFrame) {
                targetFrame = this.add(key, sourceFrame.sourceIndex)
                targetFrame.copy(sourceFrame)
            } else if (!sourceFrame) {
                this.remove(key)
            } else {
                targetFrame.copy(sourceFrame)
        ...
        this.frameTotal = texture.frameTotal
    },
})
